...
class XiaoyingApiController extends Controller
{
    ...
    public function callStudentCreate()
    {
        ...
        $has_user = (new CallStudentModel())->where(['phone'=>['eq',$student['phone']]])->find();// 是否已注册
        ...
        // 如果有来源信息
        if ($data['goto']) $student['goto'] = $data['goto'];
     ...
     * 更新打Call学员数据
     */
    public function updateCallStudentInfo()
    {
        $data = I();// 接收参数

        if (!$data || !$data['ui'] || !$data['phone']) $this->ajaxReturn(['status'=>false,'msg'=>'缺少参数']);// 缺少参数

        $has_user = (new CallStudentModel())->where(['phone'=>['eq',$data['phone']],'id'=>['eq',$data['ui']]])->find();// 是否已注册

        if ($has_user) $this->ajaxReturn(['status'=>true,'res'=>$has_user]);// 如果已注册,返回用户信息

        $this->ajaxReturn(['status'=>false,'msg'=>'用户不存在']);// 用户不存在
    ...
    public function resetShareStatus()
    {
        ...
        echo 'reset user share_status success!';
    }

    /**
     * 每天更新用户的剩余call值  (每天加5个)
     * TODO 活动结束后,可删除此方法和相关的定时任务 (活动结束时间待定)
     */
    public function addStudentCallNum()
    {
        $users = (new CallStudentModel())->select();// 获取所有用户

        if ($users)
        {
            foreach ($users as $k => $v)
            {
                $new_call_num = $v['call_num'] + 5;
                (new CallStudentModel())->where(['id'=>['eq',$v['id']]])->save(['call_num'=>$new_call_num]);
            }
        }

        echo 'update user call_num success!';
    }
