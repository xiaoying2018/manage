...
//        if (!$res) $this->ajaxReturn(['status'=>false,'msg'=>'系统维护,请稍后再试!']);// 更新失败
    ...
    public function delete()
    {
        ...
        if (in_array(session('xy_manager')['id'],$ids)) $this->ajaxReturn(['status'=>false,'msg'=>'非法操作,不能删除当前登录账号']);// 缺少关键参数
...
//        if (!$res) $this->ajaxReturn(['status'=>false,'msg'=>'系统维护,请稍后再试!']);// 更新失败
    ...
    {
        if (!$password) return md5(md5('XY-'.'invY1234'));// 生成默认密码

        return md5(md5('XY-'.$password));// 生成指定密码
    }
