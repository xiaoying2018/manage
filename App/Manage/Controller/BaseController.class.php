...
class BaseController extends Controller
{
    ...
    public function _initialize()
    {
        ...
        if (session('xy_manager')['status'] != '9') // 特殊用户跳过判断
        {
    ...
    {
        if (!session('xy_manager')) $this->error('请先登录!','/login',2);
    }
    ...
    public function hasCurrentRequestPre()
    {
        ...
        $current_user_has_pre = session('xy_manager')['has_pre'];// 当前用户已有权限
    ...
    public function getLeftNav()
    {
        ...
        // 获取当前用户角色
        $current_role = M('UserRole')->where(['user_id'=>['eq',session('xy_manager')['id']]])->find()['role_id'];
