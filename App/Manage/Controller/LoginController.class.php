...
class LoginController extends Controller
{
    ...
    {
        $password = md5(md5('XY-'.$data['password']));// 密码处理
        ...
        // 登录成功
        session('xy_manager',$user);// 存入session
    ...
    {
        session('xy_manager',null);// 清空登录状态

        if (!session('xy_manager')) $this->success('已退出当前账号','/login',1);
    }
