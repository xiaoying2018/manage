...
//            if (!$par['catename']) $this->ajaxReturn(['status'=>false,'msg'=>'分类名不能为空']);// 参数过滤
            ...
            try{
                $res = (new NewscontentsModel())->add($par);// 新增
...
//        var_dump($catedata);
    ...
    public function edit(){
        if (IS_POST && IS_AJAX)// 如果修改
        {
            ...
            $par['webs'] = implode(',',$par['webs']);
            $par['update_time'] = time();
            // 执行更新操作
            try{
                $res = (new NewscontentsModel())->where(['id'=>$par['ids']])->save($par);// 更新
            }catch (\Exception $exception){
                $this->ajaxReturn(['status'=>false,'msg'=>$exception->getMessage()]);// 捕获异常
            }
        ...
        $info = (new NewscontentsModel())->find($id);// 获取要修改的数据
    ...
    public function search(){
        if (IS_AJAX)
        {
            $contentmodel = new NewscontentsModel();
            $page = I('get.page');
            $offset = I('get.limit');
            ...
            //标签搜索
            $cid = [];
            if($stags = I('get.tagsearch')!=''){
                foreach ($contentdatas as $kk=>$vv){
                    if(strpos($vv['content'],$stags)){
                        $cid[] = $vv['id'];
                    }
                }
                $where['id'] = ['in',$cid];
            ...
            $contentdata = $contentmodel->where($where)->limit(($page - 1) * $offset, $offset)->order('create_time desc')->select();
            $tag = [];
            foreach ($contentdata as $k=>$v){
                $contentdata[$k]['faceimg'] = substr($v['faceimg'],1);
                $contentdata[$k]['create_user'] = M('user')->where(['id'=>$v['create_user']])->find()['name'];
                $contentdata[$k]['create_time'] = date('Y-m-d H:i:s',$v['create_time']);
    ...
    public function delete()
    {
        ...
        try{
            $res = (new NewscontentsModel())->where(['id'=>['IN',$ids]])->delete();// 删除
        }catch (\Exception $exception){
            $this->ajaxReturn(['status'=>false,'msg'=>$exception->getMessage()]);// 捕获异常
        }
