...
use Manage\Model\SchoolsgpModel;
use Manage\Model\TagsModel;

class SgpschoolController extends BaseController
{
    /**
     * 分类列表页
     */
    ...
    public function add(){
        ...
        if (IS_POST && IS_AJAX)// 新增
        {
            ...
            try{
                $res = (new SchoolsgpModel())->add($par);// 新增
                if($res){
...
//            $this->ajaxReturn($par);
            ...
            try{
                ...
                $res = M('schoolSingapore')->where(['school_id'=>$par['ids']])->save($par);// 更新
                if($res!==false){
                    foreach ($filearr as $k=>$v){
                        $add['file_id'] = $v;
            ...
            $this->ajaxReturn(['status'=>true,'msg'=>'修改成功!','sql'=>(new SchoolkoreaModel())->getlastSql()]);// 更新成功
        }
        ...
        $info = (new SchoolsgpModel())->find($id);// 获取要修改的数据
...
//        $this->ajaxReturn($ids);
    ...
    public function search(){
        if (IS_AJAX)
        {
            $schoolmodel = new SchoolsgpModel();
            $page = I('get.page');
            $offset = I('get.limit');
